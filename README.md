...
| 意思決定者 | 数値の出所と更新経路を短時間で把握しにくい | 8画面の公開ビューと、Actions実行・commit履歴で追跡できる更新プロセス |
...
- **優先アクション**: AI の結論・理由・次の行動を先に表示し、観測内容と順位の説明は展開して
  確認できます。Advisor の確認ガイドは「ルールベース」と明示し、AI 分析とは区別します。
...
- **推奨事項**: Advisor を内容別にまとめ、確認理由・具体的な確認事項・保留を検討できる条件を
  表示します。カテゴリは一行に集約し、影響度は内訳として表示します。件数は障害件数ではなく
  推奨レコード数です。重複を除いた対象数とは分け、未分類の内容を推測しません。
  従来のカテゴリ集計は全状態を含み、内容カードは完了・却下・延期を除いた対象です。
  「内容分類済み＋内容未確認＋詳細から除外＝全状態の集計」を画面上で照合できます。
  確認ガイドはレビュー済みカタログによるルールベースの説明で、AI が発見した事実ではありません。
- **セキュリティ**: Defender の取得状態を項目別に表示し、Advisor はセキュリティ分類のみを
  掲載します。他カテゴリは独立した「推奨事項」画面から確認できます。両者の件数は合算しません。
  Defender の公開グループ数・評価レコード数を、全推奨数・対象リソース数として表示しません。
...
AI インサイトは、根拠のある比較・偏り・関連から確認の優先順位を説明するものに限定します。
一般的な注意書き、単なる件数の言い換え、収集範囲だけの指摘は生成対象外です。
対象外・取得失敗は、AI の有無に関係なく「収集範囲・制約（自動集計）」で分離表示します。
対象外を障害や監視不備とみなさず、有用な候補がなければ 0 件を許容します。

新規 AI 候補の `--insights-only` 公開ゲートでは、通常は異なる数値根拠を 2 パス以上要求します。
具体的な内容を収集できた Advisor 推奨は、その内容に基づく条件付き判断を説明する場合に限り
数値根拠 1 パスでも対象にします。同じ件数や無関係な数値を追加して水増しする必要はありません。
取得済みの正のアラート数・状態低下数なども、具体的な切り分け判断がある場合は 1 パスで扱えます。
Defender の一部項目が未取得でも、個別に取得成功した項目まで一律に分析対象外にはしません。
評価状態に不明値が混ざっても、取得済みの正の `Unhealthy` 評価数は保持します。
不明件数や掲載範囲だけを指摘へ昇格させず、不明を正常・確認済みの問題・対応作業中と
読み替えません。評価レコード件数から重複を除いた対象数や非公開の問題内容も推測しません。
任意の `security.vulnerabilities` は独立した取得状態を持ち、実際に掲載された CVE と
対象参照・スコアなどから具体的な確認判断を説明できます。別の Defender 項目の取得失敗で
取得済みの脆弱性まで無効にせず、成功時のゼロ件も「安全」とは断定しません。
確認できた CVE の判断には実数値 1 パスでも使えますが、悪用・外部露出・業務影響・
更新の安全性は推測せず、件数だけから架空の脆弱性を補いません。
日本語監査でも、そのスナップショットに掲載された正規の CVE 識別子だけを識別子として
扱います。未観測の番号、数字の接尾辞による別番号、識別子に続く英語文は免除しません。
`unmappedTargetSubAssessments` は対象をインベントリに照合できなかった CVE 付き評価レコード数で、
未対応の内容を示す `unmappedSubAssessments` とは区別します。参照が空なら対象を推測せず、
運用担当者による外部確認へ案内します。
根拠が収集範囲やイベントの状態別総数だけではないこと、影響説明に根拠の数値を含むことも
...
対象参照や構造化アドレスは関係を確認する情報であり、文字列に含まれる数字を件数などの
数値根拠へ転用できません。Advisor の対象掲載範囲・未解決参照などの収集範囲カウンター
だけを、運用上の不具合を示す AI 分析へ昇格させることも拒否します。
数値の符号を「減少」と言い換える表現は許容しますが、その解釈は後段のレビュー対象です。
Advisor のカテゴリ件数だけによる「多いから先に確認」は拒否します。対応する具体的な
推奨内容を必須とし、影響度が低い・件数が少ないという理由だけで対応不要とは判断しません。
Advisor だけを根拠にする分析は実際の掲載先 `/recommendations` へ案内します。
この画面にない使用量や原因を観測済みとする主張は拒否しますが、追加調査の場所と不足情報を
明記した外部確認（例: Azure portal のコスト分析）は許容します。
推奨手順は操作ごとに確認場所を明記します。`/reliability` の Service Health 表示は
継続中／解決済み件数とカテゴリ集計であり、個別イベントの対象サービス・リージョンや
通知設定・テスト結果は含みません。それらは [Azure ポータルの Service Health](https://learn.microsoft.com/ja-jp/azure/service-health/service-health-overview)
で運用担当者が追加確認する情報として区別します。Advisor のガイドも実際の保護設定を
示すものではありません。生成側は各手順と画面の機能を照合してから候補を提出し、
決定論的チェック成功だけで意味レビューにも合格したとは扱いません。
デモの判断材料を減らさないよう、具体的な改善案・費用との比較・条件付きの見送り判断を
優先し、注意書きだけの文章にはしません。Azure 設定の自動変更は行いません。
生成時は、復旧・継続性・互換性・支出などの**異なる運用判断**を選び、結論・対象・根拠・
次の確認と判断条件の順に説明します。カタログの確認ガイドに件数を添えただけの文章ではなく、
このスナップショットの比較、または確認結果によって対応／保留が変わる前提条件を求めます。
同じ種類・似た別名・同じ件数だけで対象を結び付けません。デモ用途でも再生成可能性や
代替策の確認なしに「対応不要」とせず、保留するなら再評価の条件を示します。
任意の `advisor.details.groups[].resourceRefs`／`targets` は取得済みインベントリに一致する
対象の参照・種類・リージョンであり、リソースやトポロジーノードの ID と完全一致する場合だけ
関係を説明できます。掲載範囲・切り詰めと、推奨レコードのスコープ別件数はリスクとは分けます。
構造化ネットワーク情報はアドレスの観測であり、疎通・許可通信・保護設定の証明ではありません。
対象の詳細には、公開スキーマで許可した構造化構成情報だけを表示します。承認済みの
private IPv4／CIDR・部分マスク public IPv4 は表示しますが、生の構成ダンプ・アプリ設定・認証秘密は表示しません。
旧スナップショットに任意項目がない場合は不明として扱い、対象ゼロや正常へ読み替えません。
過去のスナップショットは再生成まで読み込み・ビルド可能とし、手作業で分析を書き換えません。

### 最新収集と直近の分析を分離

最新データは `public/data/snapshot.json` に公開し、生成入力から旧 `aiInsights` を除く動作は
維持します。新しい収集の前に、公開済みの非空分析を**当時のスナップショット全体**として
`public/data/last-analysis.json` に保持します。古い分析だけを新しい根拠へ移植しません。
最新の分析が空の場合、画面はこのファイルを当時の収集日時付きの過去分析として参照できます。
その対象や詳細も当時の根拠であり、現在の状態・生成中・失敗を推測する表示にはしません。

分析の保持は最大 1 MiB の単一ファイルです。独立レビューまで通過した非空分析の公開で更新し、
生成失敗・レビュー否認・ゼロ件の結果では前の有用な分析を消しません。旧分析がない場合は
通常の空状態です。保持時と CI で JSON Schema・runtime schema・日時・ID・数値根拠・
日本語・privacy を検証し、公開データ全体とビルド出力も privacy scan の対象にします。
既存ファイルの破損・非互換スキーマを無視して空状態へ落とすことはなく、公開処理は失敗します。
スキーマ変更時は旧ファイルを互換にできるか確認する必要があります。

スコープの匿名ラベルや部分マスクが同じだけでは、過去分析を現在のデータへ関連付けません。
信頼された公開処理だけが最大 2 KiB の `public/data/analysis-continuity.json` を発行し、
旧スナップショット全体と、実スコープのダイジェストを含む最新の AI 除外済み根拠を
canonical JSON の SHA-256 で結び付けます。収集ごとに設定済みの完全な tenant/subscription ID の
ダイジェストと既存の来歴を照合します。生の ID はこのファイルへ出力しません。
画面も両スナップショットのハッシュを照合し、対応ファイルの欠損・変更・古い組み合わせを拒否します。
ブラウザーの描画用形状・数値整合チェックは、公開側の完全な検証や独立レビューの再実行ではありません。
これは署名ではなく、保護された公開ワークフローを信頼元とする整合性検証です。

旧マスク形式からの初回移行は、凍結済み公開コミット `df41093` の根拠と、
読み取り確認済みの実スコープを明示的に固定した移行許可に限ります。旧分析の根拠や ID は
書き換えません。異なる実スコープへの変更は、匿名表示が同じでも公開を停止します。
...
期間・分母・増減方向の誤読、画面にないデータをあるとする主張は拒否対象です。

レビュー用モデルは `gpt-5.4` に固定し、自動ルーティングによる判定モデルの変動を避けます。
件数だけの推奨を誤って通した自動選択モデルから切り替えていますが、判定基準や合成例の
正解は変更していません。モデル固定後も、毎回の独立レビューと以下の照合を省略しません。

レビューには、同じ基準で判定する固定の合成例 9 件（適切な例 4 件、不適切な例 5 件）も
渡します。正解は入力に含めず、trusted publisher が判定結果を照合します。
内容重視の要件変更に合わせ、以前は許容していたカテゴリ件数だけの優先順位付けを不適切な例へ
変更しました。具体的な冗長化推奨の条件付き判断を適切な例、低影響だから放置する判断を
不適切な例として追加しています。また、具体的な外部調査と一部取得済みのアラートまで
過剰に排除しないことも確認します。実行結果を合格させるために判定を変更するものではありません。
...
**公開更新は人の都度承認なしで実行しますが、成功は保証しません。**
現在の定期収集は火・金 06:00 JST（cron: `0 21 * * 1,4`）であり、常時監視や連続更新では
ありません。収集・匿名化・公開前検証が失敗すれば新しい収集は公開しません。
収集公開後に AI 生成・独立レビュー・配信が失敗する場合は、収集済みデータが先に更新されて
いることがあります。その場合も旧分析は当時の根拠とともに保持し、各 run の失敗を報告します。
ブラウザーはスナップショットを自動ポーリングしません。経過時間の表示は 60 秒ごとに更新しますが、
読み込み済みデータはページの再読み込みまで変わりません。
...
- **Cost Management**: 現在/前期間を独立して取得します。空の成功応答は費用ゼロではなく未取得です。
  HTTP 429/503 は自動再試行します。`Retry-After` と Cost Management の quota 別
  `retry-after` ヘッダーの最大待機時間を守り、ヘッダーがない場合は15・30・60・120秒へ
  待機を延ばします（最大1秒のランダムな追加待機）。1ページ最大6回、1期間全ページで10分の
  予算と1要求60秒のタイムアウトを設けています。予算を超えるサーバー指定の待機を短縮して
  再要求することはありません。現在/前期間は直列に取得し、上限到達時は未取得のまま次の
  定期実行で再度取得します。認証・権限エラーは再試行しません。
  トークンは既存 OIDC の CLI セッションからメモリ上で取得し、応答ヘッダーを直接読める
  HTTP クライアントで問い合わせます。ログはステータス・再試行回数・待機秒数だけです。
  詳細は[公式のレート制限と待機指示](https://learn.microsoft.com/ja-jp/azure/cost-management-billing/costs/manage-automation#data-latency-and-rate-limits)を参照してください。
...
- **Defender for Cloud**: 評価、セキュア スコア、アクティブなアラートを独立して読み取ります。
  ...
  達成率として流用しません。
- **Azure Advisor**: `AdvisorResources` のカテゴリ・影響度別集計に加えて、
  レビュー済みの公開カタログに対応する推奨を内容別に表示します。生の推奨文・対象名/IDは
  公開せず、公開文はカタログ内の固定された日本語だけを使用します。対応しない内容は
  未分類として保持し、具体的な問題や低リスクを推定しません。レコード数と重複を除いた
  対象リソース数を区別し、対象を特定できない場合は対象数を不明にします。
  未知の分類は `Other` / `Unknown` に集約し、成功して0件だった結果は読み取り失敗と区別します。
  `Completed` / `Dismissed` / `Postponed` は内容カードから除外し、除外数を別表示します。
  ライフサイクルがないレコードは未確認として保持し、全件を未解決と断定しません。
  詳細の `partial` がカタログ未分類や対象数未確認だけを表す場合は、取得済みで内容分類済みの
  個別グループを AI の根拠にできます。収集ソース自体の失敗・一部取得は引き続き拒否します。
  ガイドは読み取り・確認の支援だけです。本番/検証の用途、停止許容、代替策、追加費用を
  観測していない限り、条件を満たす・対応不要とは判断しません。
...
advisor?: {
  ...
  details?: {
    availability, message, mappedRecommendationCount, withheldRecommendationCount,
    excludedRecommendationCount, lifecycleUnknownCount, affectedResourceCount,
    groups: [{
      id, category, contentStatus, count, impacts, affectedResourceCount, resourceTypes,
      title, description, recommendedAction, deferWhen, caveat, sourceUrl
    }]
  }
}
network.topology?: {
...
生の ARM ID／名前／構成ダンプはメモリ内だけに保ち、JSON には書き出しません。
公開する構成情報は、上記の許可済み構造化フィールドに限定します。
...
AIの数値根拠は引き続き実在する値と利用可能なソースを必須とします。
`advisor.recommendations.<index>.count` はカテゴリ・影響度の集計値であり、それだけでは
AI 分析の根拠として不十分です。具体的な判断には、内容を確認できた
`advisor.details.groups.<index>` の推奨を参照します。
